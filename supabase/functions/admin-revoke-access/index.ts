// admin-revoke-access
// Admin-only. Soft-revokes a user's subject or year access (sets revoked_at)
// — reversible, and writes an audit-log entry.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (kind === "subject") {
      ...
      await db.from("user_subject_access").update({ revoked_at: now })
        .eq("user_id", target_user_id).eq("subject_id", subject_id).is("revoked_at", null);
    } else {
      if (!year_id) return jsonResponse({ error: "year_id required" }, 400);
      await db.from("user_year_access").update({ revoked_at: now })
        .eq("user_id", target_user_id).eq("year_id", year_id).is("revoked_at", null);
    }

    await db.from("admin_audit_log").insert({
      admin_id: user.id,
      action: kind === "subject" ? "revoke_subject" : "revoke_year",
      target_user_id,
