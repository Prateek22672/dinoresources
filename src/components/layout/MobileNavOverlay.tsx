...
export default function MobileNavOverlay({
  ...
  const itemBtn = (item: MobileNavItem, i: number) => (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-[15px] font-medium text-left
        transition-all duration-300 ${open ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-3"}
        ${item.active ? "bg-white text-black font-semibold"
  ...
  return (
    <div className={`lg:hidden fixed inset-0 z-[80] ${open ? "" : "pointer-events-none"}`} aria-hidden={!open}>
      ...
      {/* rail-style drawer */}
      <aside
        className={`absolute left-3 top-3 bottom-3 w-[290px] max-w-[86vw] bg-[#131316] border border-white/8 rounded-[28px] p-4
      ...
      >
        ...
        <div className="flex items-center gap-2.5 px-2 pt-1 pb-4">
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-full text-zinc-400 hover:text-white hover:bg-white/10 flex items-center justify-center"
