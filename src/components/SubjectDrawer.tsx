...
import { List, LayoutGrid, X, BookOpen, Lock, ArrowRight } from "lucide-react";
...
import { useSubjectOwnership } from "@/hooks/useSubjectOwnership";
import { SubjectUnlockDialog } from "@/components/SubjectUnlockDialog";
...
export default function SubjectDrawer({
  ...
  const { isOwned, loading: ownershipLoading, refresh: refreshOwnership } =
    useSubjectOwnership(subjectId);

  const aiSubjectTopics = getAiSubject(subjectName) || {};
  const availableAiUnits = [1, 2, 3, 4, 5].filter(
    (u) =>
      Array.isArray(aiSubjectTopics[u.toString()]) &&
      aiSubjectTopics[u.toString()].length > 0
  );
  ...
  return (
    <>
      <Drawer open={open} onOpenChange={onOpenChange}>
        <DrawerContent className="h-[100dvh] sm:h-[94vh] max-h-[100dvh] w-full mx-auto rounded-none sm:rounded-t-[32px] overflow-hidden bg-[#0a0a0c] border-t border-white/10 text-zinc-100 flex flex-col focus:outline-none pointer-events-auto">
          ...
          <DrawerHeader className="relative shrink-0 border-b border-white/5 bg-[#0a0a0c] z-[60] px-4 py-4 flex flex-col items-center justify-center min-h-[90px]">
            ...
            {/* AI Quick Button — only show if owned so locked subjects don't advertise AI */}
            {hasAiModule && isOwned && (
              <button
          ...
          <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
            ...
            <div className="w-full md:w-64 shrink-0 border-b md:border-b-0 md:border-r border-white/5 bg-[#0d0d10] z-20 overflow-hidden">
              <nav
                className="flex md:flex-col gap-2 md:gap-1.5 px-3 py-2.5 md:p-3 overflow-x-auto md:overflow-x-hidden md:overflow-y-auto scrollbar-none snap-x snap-mandatory md:snap-none items-center md:items-stretch scroll-smooth"
                style={{
              ...
              >
                ...
                {/* Right breathing room for mobile */}
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 scrollbar-thin scrollbar-thumb-zinc-800">
                ...
                {ownershipLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center gap-4 p-12 text-center">
                    ...
                    <h3 className="text-base font-semibold text-white">{subjectName}</h3>
                    <p className="text-zinc-400 text-sm max-w-xs">
                      Unlock this subject once to permanently access all resources and AI tools.
                    </p>
                    <Button
                      onClick={() => setUnlockDialogOpen(true)}
                      className="rounded-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold px-8 py-5 shadow-lg"
                    >
                      Unlock Subject
                    </Button>
                  </div>
                  ...
                  // ── Owned content — wrapped in a fragment so siblings are valid ──
                  <>
                    ...
                    {/* AI banner — only for unit tabs that have AI content */}
      ...
      {/*
        Rendered outside DrawerContent so it isn't unmounted when the
        ownership branch switches — the Dialog portal handles its own z-index.
      */}
      ...
      <SubjectUnlockDialog
        ...
        price={subjectPrice}
        onPaymentSuccess={refreshOwnership}
      />
    </>
  );
}